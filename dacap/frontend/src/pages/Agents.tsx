...
export default function Agents() {
  ...
  const fetchPortfolio = useCallback(async (agentId: string) => {
    try {
      const res = await fetch(`http://localhost:8000/api/agents/${agentId}/portfolio`)
      if (res.ok) {
  ...
  const handleStartTrading = async (agentId: string) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/agents/${agentId}/start-trading`, { method: 'POST' })
  ...
  const handleStopTrading = async (agentId: string) => {
    setTradingLoading(prev => ({ ...prev, [agentId]: true }))
    try {
      await fetch(`http://localhost:8000/api/agents/${agentId}/stop-trading`, { method: 'POST' })
      await fetchPortfolio(agentId)
